...
All user data under `users/{uid}/`. In-memory caches (`txCache`, `accCache`, `settingsCache`, `budgetCache`, `subTypeCache`, `paydayOverridesCache`) kept in sync via `onSnapshot` listeners, initialized in `initDataListeners(uid)`. All listeners except paydayOverrides call `renderActivePage()` on change; paydayOverrides listener also calls `renderActivePage()` for UI reactivity.
...
- **Account matching**: AI returns `accountHint`; `displayParseResult()` does case-insensitive substring match against `accCache`.
